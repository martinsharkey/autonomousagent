...
def test_autonomous_mutation_cycle_proposes_visible_content() -> None:
    ...
    assert mutation.mission_pillar is not None
    assert mutation.quality_score is not None
    ...
    _write_json(
        "01_proposed_mutation.json",
        {
            ...
            "quality_score": mutation.quality_score,
    ...
    _append_report(
        ...
        f"- **Agent**: {mutation.agent_name}\n"
        f"- **Description**: {mutation.description}\n"
        f"- **Proposed Changes**: {json.dumps(mutation.proposed_changes)}\n"
        f"- **Quality Score**: {mutation.quality_score}\n"
        f"- **Status**: {mutation.status.value}\n",
    )
...
def test_autonomous_mutation_cycle_consensus_and_approval() -> None:
    ...
    mutation = propose_mutation(
        agent_name="autobot",
        mutation_type=MutationType.PARAMETER_ADJUSTMENT,
        description="Optimize self-evolution parameters to improve learning feedback",
        rationale="Current responses are too random for deterministic tool use; optimize determinism for self-improvement",
        proposed_changes={"temperature": 0.15, "max_retries": 4},
        expected_improvement=0.1,
        risk_level="medium",
    )
    ...
    assert engine.get_mutation(mutation.mutation_id).status == MutationStatus.APPROVED
    ...
    _append_report(
        ...
        f"- **Mutation Status**: {engine.get_mutation(mutation.mutation_id).status.value}\n",
    )
...
def test_autonomous_mutation_cycle_implements_code_change_and_evidence() -> None:
    ...
    mutation = propose_mutation(
        ...
        description="Optimize self-evolution by adding autonomous test marker",
    ...
    proposal_id = f"consensus-{mutation.mutation_id[:12]}"
    consensus = ConsensusEngine(agents=COUNCIL)
    consensus.create_proposal(proposal_id, mutation.agent_name, mutation.description, mutation.proposed_changes)
    for voter in COUNCIL:
        consensus.cast_vote(proposal_id, voter, "approve", f"{voter} approves autonomous marker mutation.")
    assert consensus.check_consensus(proposal_id) == "approved"

    approved = approve_mutation(mutation.mutation_id, approved_by="consensus")
    assert approved is True
    m = engine.get_mutation(mutation.mutation_id)
    assert m.status == MutationStatus.APPROVED
    ...
    _write_json(
        "03_implementation_result.json",
        {
            "mutation_id": mutation.mutation_id,
            "approved": approved,
            "status_after_approval": m.status.value,
