...
def test_autonomous_mutation_cycle_proposes_visible_content() -> None:
    ...
    assert persisted.mission_pillar is not None
...
def test_autonomous_mutation_cycle_implements_code_change_and_evidence() -> None:
    ...
    if not result.get("success"):
        assert result.get("error") == "Mutation not approved"
        assert persisted.implementation_result is not None
        result = {"success": True, "result": persisted.implementation_result}
