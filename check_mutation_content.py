...
for f in files:
    ...
    change_keys = list(changes.keys()) if isinstance(changes, dict) else 'N/A'
    print(f'--- {name} ---')
    print(f'  Status: {status}')
    print(f'  Type: {mtype}')
    print(f'  Desc: {desc}')
    print(f'  Rationale: {rationale}')
    print(f'  Changes keys: {change_keys}')
    if isinstance(changes, dict) and 'file_changes' in changes:
