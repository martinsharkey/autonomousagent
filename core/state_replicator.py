...
class StateReplicator:
    ...
    def verify(self, db_file: str) -> Dict[str, Any]:
        ...
        for replica_path in self.replicas:
            candidate = replica_path / db_file
            detail = {"path": str(candidate), "exists": candidate.exists()}
            if not candidate.exists():
                detail["status"] = "missing"
                issues.append(f"missing replica: {candidate}")
            elif source_hash and self._compute_db_hash(candidate) != source_hash:
    ...
    def heal(self, db_file: str) -> Dict[str, Any]:
        ...
        for replica_path in self.replicas:
            candidate = replica_path / db_file
            try:
                if not candidate.exists() or self._compute_db_hash(candidate) != source_hash:
                    candidate.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(source, candidate)
                    healed.append(str(candidate))
            except Exception as exc:
