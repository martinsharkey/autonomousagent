...
class TelegramBot:
    ...
    async def send_mutation_notification(self, mutation_id: str, status: str, agent_name: Optional[str] = None, speaker: str = "EVOLUTION", mutation: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if mutation:
            ...
            if mutation.get('proposed_changes'):
                changes = mutation['proposed_changes']
                if isinstance(changes, dict):
                    summary = ", ".join(f"{k}={v}" for k, v in list(changes.items())[:4])
                    body += f"<b>Proposed Changes:</b> {summary}\n"
                else:
                    body += f"<b>Proposed Changes:</b> {str(changes)[:200]}\n"
            ...
            if mutation.get('expected_improvement'):
                body += f"<b>Expected Improvement:</b> {mutation['expected_improvement']}\n"
            ...
            if mutation.get('implementation_result'):
                ...
                if isinstance(result, dict):
                    ...
                    if result.get('promotion'):
                        body += f"<b>Promotion:</b> {result['promotion']}\n"
                    if result.get('score_improvement'):
                        body += f"<b>Score Improvement:</b> +{result['score_improvement']:.1%}\n"
